...
describe('randomString()', function () {
    ...
    it('returns an error on invalid bits size', function (done) {
        ...
        var errorMessage = /(Failed generating random bits: Argument #1 must be number > 0)|(Failed generating random bits: size must be a number >= 0)/;

        expect(Cryptiles.randomString(99999999999999999999).message).to.match(errorMessage);
        done();
    });
});
